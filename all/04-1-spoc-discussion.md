...
1. 寄存器、高速缓存、内存、外存的访问特征？
   ...
   寄存器直接访问，高速缓存经过虚实地址转换后直接查找访问，内存经过虚实地址转换后经过tlb查找访问访问，外存需要使用中断访问。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

   每一层既提高了效率，又没有使用过多额外空间。
...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   ...
   覆盖技术是划分程序模块，不同模块使用同一块内存的技术。程序开发者的主要工作是模块划分和确定模块的共享关系。

2. 什么是交换技术？覆盖与交换有什么不同？

   交换技术是作为一个整体的进程地址空间的换入和换出。覆盖的思路是是切块，交换是进程整体交换。
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   时间局部性是指被访问过的地址在未来很可能会再被访问，空间局部性是被访问的地址附近的地址可能会被访问，分支局部性是分支指令跳转较多的选择很可能会再次被选择。
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   虚拟存储是指只把当前访问的内容快读入内存。和覆盖的区别是虚拟存储不是功能模块的划分，和交换的区别是虚拟存储把程序分块。好处：空间大，地址不必连续。挑战：需要地址转换。
...
2. 为了支持虚拟页式存储的实现，页表项有什么修改？
 ...
 3. 页式存储和虚拟页式存储的区别是什么？
    ...
    虚拟页式存储的空间更大，页可能缺失，此时要执行中断指令。

### 8.6 缺页异常

1. 缺页异常的处理流程？

   在外存中查找需要访问的页面；页面读入内存；修改页表项；重新执行导致缺页异常的指令。


2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

   设缺页率为p，有效访问时间=访存时间*（1-p）+缺页处理时间*p
